...
const generalizeDiagnosis = () => import('components/diagnosis/generalizeDiagnosis.vue');
Vue.use(Router);
...
const vueRouter = new Router({
  routes: [
    ...
    {
      path: '/generalizeDiagnosis',
      name: 'generalizeDiagnosis',
      component: generalizeDiagnosis,
      meta: {
        title: '基诊室',
        loadingName: 'diagnosisLoad',
        loadingTimes: 'all',
...
vueRouter.beforeEach((to, from, next) => {
  ...
  if (window._hmt) {
    window._hmt.push([
      '_trackPageview',
      window.location.href.replace(window.location.origin, '').replace(window.location.hash, '') + '#' + to.fullPath
    ]);
  }

  // utm_source 用户来源
  if (from.query.utm_source && (to.query.utm_source == '' || !to.query.utm_source)) {
    let querys = {
      utm_source: from.query.utm_source ? from.query.utm_source : ''
    };
    querys = {...to.query, ...querys};
    next({path: to.path, query: querys});
  } else {
    next();
  }
});
...
vueRouter.beforeResolve(function (to, from, next) {
  ...
  if (to.query.utm_source && to.query.utm_source != '') {
    sessionStorage.setItem('utm_source', to.query.utm_source);
  }
