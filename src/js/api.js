import {getLoginCode, loginVerification} from 'js/request.js';
 ...
 * @DateTime  2018-02-01
 * @copyright [LEAD]
 * @param     {Number}   options.type            [form表单事件 1是输入事件 2是获取动态验证码事件 3是登录或注册事件]
 * @param     {[type]}   options.phoneNumber     [手机号]
 * @param     {[type]}   options.imgCodeNumber   [图形验证码]
 * @param     {[type]}   options.phoneCodeNumber [动态验证码]
 * @param     {[type]}   options.toast           [是否拥有抛错权限  1有 0没有]
 * @return    {[type]}                           [返回一个Promise]
 */
export const registerFormCheck = function ({
  type = 1,
  phoneNumber,
  imgCodeNumber,
  phoneCodeNumber,
  ...
  return new Promise(function (resolve, reject) {
    ...
    // 输入框事件
    if (type === 1) {
      if (phoneNumber && phoneReg.test(parseInt(phoneNumber)) && imgCodeNumber && imgCodeNumber.length === 5) {
        response.canGetPhoneCodeFlag = 1;
        if (phoneCodeNumber && phoneCodeNumber.length === 6) {
          response.canRegisterFlag = 1;
        } else {
          response.canRegisterFlag = 0;
        }
      } else {
        response.canGetPhoneCodeFlag = 0;
        response.canRegisterFlag = 0;
      }
      resolve(response);
    }
    ...
    // 基础校验
    let checkPhoneImgCode = function () {
      if (phoneNumber == '' && imgCodeNumber == '' && phoneCodeNumber == '') {
        toast('请输入手机号码');
      } else if (phoneNumber.length < 11 || !phoneReg.test(parseInt(phoneNumber))) {
        toast('请输入有效的手机号');
      } else if (imgCodeNumber.length < 5) {
        toast('图形验证码不正确');
      } else {
        return true;
      }
    ...
    // 获取手机验证码事件
    if (type === 2) {
      if (checkPhoneImgCode()) {
        getLoginCode({
          'phone': phoneNumber,
          'verifyPicNo': imgCodeNumber
        }).then(function (resp) {
    ...
    if (type === 3) {
      if (checkPhoneImgCode()) {
        if (phoneCodeNumber.length < 6) {
          toast('手机动态码不正确');
        } else {
          loginVerification({
            'phoneNO': phoneNumber,
            'verifyPicNo': imgCodeNumber,
            'verifySMSNo': phoneCodeNumber
          }).then(function (resp) {
            if (resp.data.respCode == '000') {
              // 储存登陆状态
              store.commit('UPDATE_LOGINSATE', 1);
