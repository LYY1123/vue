    ...
    fundDetails (id) {
      ...
      diagnosisDetail('', fundId).then(function (resp) {
        ...
        self.$nextTick(function () {
          self.fundDataReady = 1;
          if (self.advertDataReady == 1) {
            self.pageAlready = 1;
          }
        });
      });
      ...
      advertising().then(function (resp) {
        if (resp.data.respCode == '000') {
          ...
          for (var i = 0; i < advice_group.length; i++) {
            if (advice_group[i].code === "advice") {
              self.advert = advice_group[i].advice_group1;
              break;
            }
          }
          ...
          // 页面加载完毕判断
          self.$nextTick(function () {
            self.advertDataReady = 1;
            if (self.fundDataReady == 1) {
              self.pageAlready = 1;
            }
          });
        }
